...
--Query 3: coordinator who managed the max number of gatherings last month 
SELECT TOP 1
    s.fname + ' ' + s.lname AS CoordinatorName,
    COUNT(*)                 AS TotalGatherings
FROM Staff s
JOIN GatheringStaff gs ON s.staff_id     = gs.staff_id
JOIN Gathering      g  ON gs.gathering_id = g.gathering_id
WHERE gs.role       = 'Coordinator'
  AND MONTH(g.date) = MONTH(GETDATE()) - 1
  AND YEAR(g.date)  = YEAR(GETDATE())
GROUP BY s.staff_id, s.fname, s.lname
ORDER BY TotalGatherings DESC;
